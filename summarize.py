...
TRANSFORM_LUT = {
    ...
    'mflops':   lambda n, t: 2 * (n ** 3) / (t * 1000000)
}
...
for i, p in enumerate(problems):
    ...
    if m:
        desc = m[1]
        if desc.startswith('Basic implementation'):
            desc = 'Basic dgemm'

        if desc == 'Blocked dgemm':
            for l in p[1:]:
                v = re.search('Problem size N\=(\d+), block size b\=(\d+), took (\d+\.\d+) seconds', l)
                if v:
                    n = int(v[1])
                    b = v[2]
                    t = float(v[3])
                    descb = f'Blocked dgemm; b={b}'
                    if not descb in data.keys():
                        data[descb] = {}
                    if not n in ns:
                        ns.append(n)
                    data[descb][n] = TRANSFORM_LUT[args.transformation](n, t)
